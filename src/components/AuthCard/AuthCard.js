    ...
    <div className={styles.card}>
      ...
      {!loading && user && (
        <>
          ...
          {!hasFarm && (
            <button onClick={onBuyFarm} className={styles.farmButton} disabled={buyingFarm || (money ?? 0) < 500}>
              {buyingFarm ? he.authBuyFarmBuying : he.authBuyFarm}
            </button>
          )}

          {!isVIP && (
            <button onClick={onBuyVip} className={styles.vipButton} disabled={!onBuyVip || (money ?? 0) < 1500}>
              {he.authBuyVip}
            </button>
          )}
